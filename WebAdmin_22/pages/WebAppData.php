...
if($P[0]["ROLE"]==3){
   $quetFt= " WHERE PAGE_SOURCE <> 'DerekLink.php' KD_USER=".GetSQLValueString($Congis,$P[0]["KD_USER"],"int");
}
 if($P[0]["ROLE"]==2){
...
function GetDataGroupLayers(){
  var pg = eval(document.getElementById("CboAp").value) ;	
	$("#loding").show();
	$.ajax({
	url: "<?php echo $nama_folder; ?>/CboList-WebAppGroup.jsp/",
	data: "model="+pg,
	cache: false,
	success: function(msg){
		 document.getElementById("KDGROUP").innerHTML = '<option value="0">--- Tidak ada ---</option>'+msg;
		$("#loding").hide();
		}
	});
};
                        ...
                        <div class="panel-body">
                             <div class="form-group">
                               <label>Nama App</label>
                             	<select name="CboAp" class="form-control" id="CboAp" onchange="TampilkanTabel()">
                                  <option value="0">---- Pilih Web GIs App -----</option>
                             	  <?php do { ?>
                            ...
                            <div class="form-group">
                      			<label>Group Layer</label>
                      			 <select name="KDGROUP" id="KDGROUP" class="form-control">
